...
const KPI_INFO = {
  fleet: "Total number of autonomous agents currently registered and their synchronization status.",
  load: "Number of active task threads currently being processed across the neural grid.",
  burn: "Monthly API infrastructure costs and real-time budget utilization tracking.",
  pulse: "Total gross revenue generated by the system through successful project completions."
}
...
function TerminalFeed({ events }: { events: SystemEventWithContext[] }) {
  return (
...
export function Overview() {
  ...
  const { data: events, loading: eLoad } = useEventsWithContext(20)
  const { state,        loading: sLoad } = useProjectState()
  const { runCounts, lastRuns, loading: stLoad } = useAgentStats()
  const { data: payments                } = usePayments()

  const [selectedAgent, setSelectedAgent] = useState<Agent | null>(null)

  const loading = aLoad || tLoad || eLoad || sLoad || stLoad
  ...
  const activityFrequency = useMemo(() => {
    const runsList = Object.values(lastRuns).flat()
        ...
        <div className="group relative rounded-2xl border border-white/5 bg-white/[0.02] p-5 transition-all hover:bg-white/[0.04] z-10 hover:z-50">
          ...
          <div className="flex justify-between items-start mb-4 relative z-10">
            <div className="flex items-center gap-2">
              ...
              <InfoTooltip text={KPI_INFO.fleet} />
            </div>
        ...
        <div className="group relative rounded-2xl border border-white/5 bg-white/[0.02] p-5 transition-all hover:bg-white/[0.04] z-10 hover:z-50">
          ...
          <div className="flex justify-between items-start mb-4 relative z-10">
            <div className="flex items-center gap-2">
              ...
              <InfoTooltip text={KPI_INFO.load} />
            </div>
        ...
        <div className="group relative rounded-2xl border border-white/5 bg-white/[0.02] p-5 transition-all hover:bg-white/[0.04] z-10 hover:z-50">
          ...
          <div className="flex justify-between items-start mb-4 relative z-10">
            <div className="flex items-center gap-2">
              ...
              <InfoTooltip text={KPI_INFO.burn} />
            </div>
        ...
        <div className="group relative rounded-2xl border border-white/5 bg-white/[0.02] p-5 transition-all hover:bg-white/[0.04] z-10 hover:z-50">
          ...
          <div className="flex justify-between items-start mb-4 relative z-10">
            <div className="flex items-center gap-2">
              ...
              <InfoTooltip text={KPI_INFO.pulse} />
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="h-[450px] bg-white/[0.02] border border-white/5 rounded-2xl p-6 overflow-hidden flex flex-col relative group">
          ...
          <div className="flex items-center justify-between mb-6 relative z-10">
            ...
            <span className="text-[9px] font-mono text-slate-600 uppercase tracking-widest">Node Select Active</span>
          </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {teamAgents.map(a => (
                      <button 
                        ...
                        className="flex items-center gap-2 p-1.5 rounded-lg bg-black/20 border border-white/[0.02] transition-all hover:border-[#00D4FF]/30 hover:bg-[#00D4FF]/5 group/node text-left"
                      >
                        <span className={clsx("w-1.5 h-1.5 rounded-full flex-shrink-0", STATUS_DOT[a.status])} />
                        <span className="text-[10px] font-mono text-slate-400 truncate group-hover/node:text-white transition-colors">{a.name}</span>
                      </button>
                    ))}
                  </div>
                </div>
