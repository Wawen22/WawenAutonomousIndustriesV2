...
- **AUTOMATION RUNTIME** nuovo `backend/src/services/personal-automation.ts`: scheduler founder locale per `Daily Founder Brief`, stato persistente in `workspace/personal/neb/automations.json`, default `disabled`, supporto `Run now`, `enabled/disabled`, ultimo run, ultimo output e prossimo run pianificato
- **SHARED EXECUTION PATH** nuovo `backend/src/services/personal-assistant-actions.ts`: dashboard quick actions e automazioni founder riusano lo stesso percorso `runCeoNaturalLanguageHandler()` invece di duplicare logica
- **BACKEND API** nuovi endpoint locali: `GET /api/personal/automation/status`, `POST /api/personal/automation/config`, `POST /api/personal/automation/run`
- **ASSISTANT HQ** nuovo pannello `Automation Control`: toggle persistente per spegnere/riaccendere l’automazione, `Run now`, stato runtime, ultimo run e output più recente
- **VERIFY** typecheck verde backend/dashboard; smoke test reale ok su `status`, `enable/disable` e `run now`, con `daily-founder-brief-2026-03-19.md` rigenerato correttamente anche a automazione disattivata
